...
use grammers_client::client::{DownloadIter, LoginToken, PasswordToken};
...
impl Client for GrammersClient {
    ...
    async fn download(&self, chat_id: i64, message_id: i64, dir: &str) -> Result<DownloadOutcome, ClientError> {
        ...
        // 缓存走与在线播放完全相同的拉流流程（iter_download 整段 + 自管写盘），
        // 替代 `message.download_media`（其落盘路径对 Windows 文件名/目录敏感，
        // 曾在文件名含非法字符时统一报 os error 5）。播放正常的媒体即可缓存。
        let mut iter = self.inner.iter_download(&media);
        iter = iter.chunk_size(RANGE_CHUNK);
        let mut body = download_range_stream(iter, 0, None);
    ...
    async fn media(
        ...
        iter = iter.chunk_size(RANGE_CHUNK);
        if boundary > 0 {
            iter = iter.skip_chunks((boundary / chunk) as i32);
        }

        let body = download_range_stream(iter, skip_lead, take);
        Ok(MediaStream {
            content_type,
            total_size,
            start,
        ...
        |mut st| async move {
            loop {
                ...
                if st.pos < st.buf.len() {
                    let avail = st.buf[st.pos..].len();
                    let n = match st.take {
                        Some(t) => {
                            let n = avail.min(t as usize);
                            st.take = Some(t - n as u64);
                            n
                        }
                        None => avail,
                    };
                    let out = st.buf[st.pos..st.pos + n].to_vec();
                    st.pos += n;
                    return Some((Ok(out), st));
                }
                // 缓冲耗尽，取下一块。
                let next = match st.iter.as_mut() {
                    Some(i) => i.next().await,
                    None => return None,
                };
                match next {
                    Ok(Some(chunk)) => {
                        let chunk = if st.skip_lead > 0 {
                            let d = (st.skip_lead as usize).min(chunk.len());
                            st.skip_lead -= d as u64;
                            chunk[d..].to_vec()
                        } else {
                            chunk
                        };
